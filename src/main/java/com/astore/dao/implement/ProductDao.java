...
//          product.setSaleRate(rs.getDouble("phan_tram"));
    ...
    public List<Product> getListProductOrder(int idOrder) {
        ...
        try{
            Connection conn = ConnectDB.getInstance();
            String sql = "SELECT SAN_PHAM.*,MAU_SAC.ten_mau_sac, MAU_SAC.ma_mau_sac_hex, DONG_SAN_PHAM.ten_dong_san_pham, h.link_hinh_san_pham\n" +
                    "FROM CHI_TIET_HOA_DON join SAN_PHAM on SAN_PHAM.id=CHI_TIET_HOA_DON.id_san_pham join MAU_SAC on MAU_SAC.id = SAN_PHAM.id_mau_sac join DONG_SAN_PHAM on SAN_PHAM.id_dong_san_pham = DONG_SAN_PHAM.id join HINH_SANPHAM h on h.id_san_pham= SAN_PHAM.id\n" +
